...
//add user
    ...
    let askOrgName = (response, convo) => {

        convo.ask("Which workshop org do you want to add user?", (response, convo) => {
            orgName = response.text;
            askUser(response, convo);
            convo.next();
        });
    };

    let askUser = (response, convo) => {

        convo.ask("Whats the user email address?", (response, convo) => {
	    //const regex = /(?<=\:)(.*?)(?=\|)/gm;
            wsUser = response.text;
...
let askOrgName = (response, convo) => {
    ...
    convo.ask("Which workshop org do you want to remove user?", (response, convo) => {
        orgName = response.text;
        askUser(response, convo);
        convo.next();
    });
...
bot.startConversation(message, confTask);
  ...
  function addUser(orgName, wsUser, callback) {
    var jsonWKS = require("/data/bender/botkit-starter-slack/json/workshop.json");
     var regex = /\:(.*?)\|/;
