...
module.exports.use = function(controller) {
  ...
  function gatherStatus(bot, message) {
    bot.startConversation(message, function(err, convo) {
      ...
      convo.ask('Do you have any Technical Validation Updates?', convoCallback, {
        key: 'updates',
        multiple: true
      });

      convo.ask('Anything blocking your progress?', convoCallback, {
        key: 'obstacles',
        multiple: true
      });
      ...
      convo.on('end', function(convo) {
        if (convo.status === 'completed') {
          standupModel.addStatus({
            updates: convo.extractResponse('updates'),
